...
- SettlementData(정산) ← FK branch_id, rider_id
...
- SettlementData: work_date 필수/DATE, delivery_count INT>=0, delivery_fee INT>=0, total_pay 계산 결과와 음수 여부 체크.
...
- 주간 집계: SettlementData를 주 단위 그룹 → PayslipGenerated 생성.
- 발송: PayslipGenerated.id 기반으로 MessageLog 생성 후 상태 업데이트.
- 지사별 필터: 모든 쿼리는 branch_id 스코프(RLS).

## 인덱스 제안

- SettlementData: (branch_id, work_date), (rider_id, work_date)
